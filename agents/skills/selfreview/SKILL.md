...
- Comments the diff adds that restate the code, narrate the change, describe old behavior, or head a section: every one is a deletion. Mechanical check for the narration cases:

  ```bash
  git diff <base> | grep '^+[^+]' | cut -c2- | node ~/.dotfiles/agents/skills/humanizer/scripts/prose-lint.js --mode comment
  ```
